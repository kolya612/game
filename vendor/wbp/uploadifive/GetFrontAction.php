...
class GetFrontAction extends \yii\base\Action
{
    /**
     * Csrf Verify Enable
     * @var bool
    ...
    public function run()
    {
        ...
        if ($itemId) {
            $progress=Progress::findOne($itemId);
            if($progress->member_id!=Yii::$app->user->identity->id) return false;
        }
        ...
        if ($images) {
            foreach ($images->each() as $image) {
                ...
                echo Html::tag('div', $content, ['class' => 'image', 'style' => 'background-image: url('.$image->getUrl($data['size'], $method).'&unique_id='.$unique_id.');','data-key'=>$image->id]);
