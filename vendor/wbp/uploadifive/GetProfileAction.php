...
class GetProfileAction extends \yii\base\Action
{
    /**
     * Csrf Verify Enable
     * @var bool
    ...
    public function run()
    {
        ...
        if ($itemId) {
            if($itemId!=Yii::$app->user->identity->id) return false;
        }
        ...
        if ($images) {
            foreach ($images->each() as $image) {
                ...
                echo Html::tag('div', $content, ['class' => 'image', 'style' => 'background-image: url('.$image->getUrl($data['size'], $method).');','data-key'=>$image->id]);
