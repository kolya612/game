...
class Image extends WbpActiveRecord
{
    ...
    public function getUrl($size = false, $method=3, $script=true){
//        $urlSize = ($size) ? '_'.$size : '';
...
//        $urlSize = ($size) ? $size : false;
        ...
        if(!$size || $this->ext=='svg') return $origin;
...
//
    public function getContent($size = false, $method=1){
        if(file_exists($this->getPath($size, $method)))
            return file_get_contents($this->getPath($size, $method));
        else
            return '';
    }    
    
    public function sendContent($size = false, $method=1){
        if(file_exists($this->getPath($size, $method))) {

            $fileHeader = fopen($this->getPath($size, $method), 'r', false);
